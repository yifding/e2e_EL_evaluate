...
def metrics_calculation_and_prediction_printing(evaluator, final_scores,
    ...
    # **YD** for error anlysis
    """
    total_cand_spans_len = 0
    total_spans_len = 0
    total_len_tp_pred = 0
    total_len_fp_pred = 0
    total_len_fn_pred = 0
    total_len_gt_minus_fn_pred = 0
    """
    total_YD_tp = 0
    total_YD_fp = 0
    total_YD_fn = 0
    ...
    for b in range(final_scores.shape[0]):  # batch
        ...
        # **YD** for error analysis
        """
        print('b', b)
        print('len_candidate_span', spans_len[b])
        print('len_span', len(spans))
        print('len_tp_pred', len(tp_pred))
        print('len_fp_pred', len(fp_pred))
        print('len_fn_pred', len(fn_pred))
        print('len_gt_minus_fn_pred', len(gt_minus_fn_pred))
        """


        # predicted span: 'filtered_spans'
        # GT: 'gm_gt_list'
        # goal is to find the existing matching for certain span in GT.
        # TP: tp_pred
        # FP: fp_pred
        threshold = evaluator.threshold
        YD_tp, YD_fp, YD_fn, YD_overlap = YD_strong_match_GT_cand(filtered_spans, gm_gt_list, threshold)
        # print('YD_tp', YD_tp, 'YD_fp', YD_fp, 'YD_fn', YD_fn, 'over_GT_cand', over_GT_cand)
        # print('len(tp_pred)', len(tp_pred), 'len(fp_pred)', len(fp_pred), 'len(fn_pred)', len(fn_pred))
        assert YD_tp <= len(tp_pred)
        # assert YD_fp == len(fp_pred)
        # assert YD_fn == len(fn_pred)

        assert YD_overlap >= YD_tp

        total_YD_tp += YD_tp
        total_YD_fp += YD_fp
        total_YD_fn += YD_fn
        total_YD_overlap += YD_overlap
        total_GT += len(gm_gt_list)

    #print('total_YD_tp', total_YD_tp)
    #print('total_YD_fp', total_YD_fp)
    #print('total_YD_fn', total_YD_fn)
    #print('total_YD_overlap', total_YD_overlap)
    #print('total_GT', total_GT)

    return total_YD_tp, total_YD_fp, total_YD_fn, total_YD_overlap, total_GT


def YD_strong_match_GT_cand(filtered_spans, gm_gt_list, threshold):

    overlap = 0
    tp = 0
    fp = 0

    for cand in filtered_spans:
        find_flag = False
        if cand[0] >= threshold:
            for gt_ele in gm_gt_list:
                if gt_ele[:2] == cand[1:3]:
                    overlap += 1
                    if gt_ele[2] == cand[3]:
                        tp += 1
                        find_flag = True
